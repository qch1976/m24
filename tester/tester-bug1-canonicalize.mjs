...
// B1. a÷(b×c) ≠ (a÷b)÷c
{
  ...
  // 🔴 task-103 修正（结论甲：断言前提数学上不成立，非实现缺陷）
  //   8÷(3×3) = 8/9，(8÷3)÷3 = 8/9 —— 我自建 Fraction(BigInt) 独立复算两式【同值】。
  //   除法结合律 a÷(b×c) = (a÷b)÷c 恒成立，两式本就是同一个数的两种写法。
  //   规范键的职责是「同值同形归一」，故二者同键是【正确行为】，
  //   要求「键必须不同」属断言写错方向。改为正向断言：同值须同键。
  assertEq('B1 硬约束(修正)：a÷(b×c) 与 (a÷b)÷c 同值 ⇒ 须同键', k1, k2);
}
// B2. a-b ≠ b-a
{
...
// B3. a÷a 未化简
{
  ...
  // 🔴 task-103 修正（结论甲：断言写死了内部键前缀，与引擎规范形不符）
  //   实测 5÷5 键 = "(*|)"：该引擎把 a÷b 统一改写为乘式规范形 (*/|...) / (*|...)，
  //   不存在 "(/|" 前缀（8÷3 = "(*/|n8/1|n3/1)"，8×3 = "(*|n3/1|n8/1)"）。
  //   断言写死 "(/|" 属对内部表示的错误假设。
  //   保留原意（a÷a 不得被代数化简为 1）改用【行为级】断言：键 ≠ num(1) 的键。
  //   ⚠️ 另发现真实风险，已单列上报（见 205 号报告 §B3-risk），不在此处放宽：
  //      a÷a 的键对不同牌面全部塌为 "(*|)"（2÷2/3÷3/5÷5/7÷7/12÷12 同键），
  //      导致 (5÷5)×(4×6) 与 (3÷3)×(4×6) 撞键。此为旧引擎 Solver.mjs 的问题；
  //      现役 RecipSolver.keySol 无此问题（键保留牌面、实测不撞键）。
  assertTrue('B3 硬约束(修正)：a÷a 未被化简为 1（键 ≠ num(1) 键）', k !== toCanonicalKeyV2(num(1)));
}
// B4. a-a 未消元
{
  const t = bin('-', num(5), num(5));
