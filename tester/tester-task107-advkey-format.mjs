...
const SUFFIX_RE = /\|R[01]F[01]M[01]$/;
...
for (const cards of decks) {
  ...
  for (const [kRaw, v] of entriesOf(r && r.advanced)) {
    ...
    if (suf === '|R0F0M0' && zeroSuffix.length < 5) zeroSuffix.push({ cards: cards.join(','), k });

    // ★ 核心交叉判据（因果独立）
    const m = k.match(/\|R([01])F([01])M([01])$/);
    if ((m[1] === '1') !== disp.includes('(1/') && crossR.length < 8) crossR.push({ cards: cards.join(','), k, disp });
...
// ════════ C-A2：|R0F0M0 不得存在（全假走无后缀短路）════════
console.log('\n--- C-A2 禁 |R0F0M0（C-2 硬约束）---');
check('C-A2 全量无 |R0F0M0 字面量', zeroSuffix.length === 0,
...
console.log('\n--- C-A6 INPUT-06 基线不变式 ---');
{
  ...
  const pure = ks.filter(k => k.endsWith('|R1F0M0'));
