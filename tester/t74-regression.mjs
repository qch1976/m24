...
// ── R-11④ 双列基准（INPUT-06.md L180 R-11④ 条文 + L245-255 明细表，两处一致）──
// 🔴 task-131 第 3 批修正（经理已批）：原写死值三处高级列各多记 1，三方对齐如下：
//     deck        INPUT-06 基准   原判据写死   实测 counts()
//     [1,3,4,6]      1/3            1/4          1/3
//     [3,3,8,8]      1/6            1/7          1/6
//     [2,4,5,8]      7/2            7/3          7/2
//   ⇒ INPUT-06 与实测【一致】，仅判据抛错 ⇒ 属【判据写死值错】，
//     非产品缺陷、非口径演进（故不需项目主重新批准基准）。
// 🔴 取值命令（现取，可复现）：
//     cd /tmp/w/b3 && node --import ./tester/render-smoke/esm-hooks.mjs tester/t74-regression.mjs
//     判据口径：counts() 内部用 reduceToFixpoint(t) 归约到不动点，再以 keySol(rr.node) 为键去重；
//     countRecip(rr.node) > 0 入 advanced，否则入 primary（见本文件 counts() 实现）。
//     数据源为 js/core/RecipSolver.mjs，与 Solver.findSolutions / findSolutionsWithAST 双 API 争议无关。
const BASE=[
 [[1,2,3,4],3,4],[[2,3,4,6],10,10],[[1,3,4,6],1,3],[[1,5,5,5],1,1],
 [[3,3,8,8],1,6],[[1,2,5,10],2,1],[[1,1,3,8],1,0],[[1,4,6,8],3,1],
 [[2,4,5,8],7,2],[[5,5,5,5],1,0],[[1,1,2,9],1,0],[[3,3,7,7],1,0],
