...
}
{
  ...
  const pick = (m) => Object.keys(DgJs).find((k) => typeof DgJs[k] === 'function' && /generate|deal/i.test(k));
  const fnName = pick();
  if (fnName) {
    for (const m of modes) {
      sameBehavior(`DealGenerator.${fnName}('${m}') 返回张数`,
        () => { const r = DgJs[fnName](m); return Array.isArray(r) ? r.length : typeof r; },
        () => { const r = DgMjs[fnName](m); return Array.isArray(r) ? r.length : typeof r; });
    }
  } else {
    ck('DealGenerator 找到可调用入口', false, `导出=[${Object.keys(DgJs).join(',')}]`);
  }
}
