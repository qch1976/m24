...
// ★ task-80 口径更新：负负得正等价（分子分母同时取反）现已归并，故 3 组高级解各 -1。
//   受影响：[1,3,4,6] 4→3、[3,3,8,8] 7→6、[2,4,5,8] 3→2；高级合计 31→28；初级合计 34 不变。
//   每组消失的那一条均已实证为【保留条的分子分母同时取反书写】、数值同为 24/1：
//     [1,3,4,6] (3-1)÷((1/4)-(1/6)) ≡ (1-3)÷((1/6)-(1/4))
//     [3,3,8,8] (8-3)÷((1/3)-(1/8)) ≡ (3-8)÷((1/8)-(1/3))
//     [2,4,5,8] (5-2)÷((1/4)-(1/8)) ≡ (2-5)÷((1/8)-(1/4))
//   ⇒ 原 §6 数字是在「未归并负负得正」的缺陷口径下统计的，含重复计数。
//   依据：项目主 2026-08-05 19:52 反例 1 + 测试 task-82 门禁 2.3/2.4/额外.2。
const BASE = [
  // [cards, primary, advanced]
  [[1, 2, 3, 4], 3, 4], [[2, 3, 4, 6], 10, 10], [[1, 3, 4, 6], 1, 3],
  [[1, 5, 5, 5], 1, 1], [[3, 3, 8, 8], 1, 6], [[1, 2, 5, 10], 2, 1],
  [[1, 1, 3, 8], 1, 0], [[1, 4, 6, 8], 3, 1], [[2, 4, 5, 8], 7, 2],
...
ck('汇总 高级合计=28（task-80 去重后，原 31 含 3 条负负得正重复）', sumA === 28, `实测 ${sumA}`);
